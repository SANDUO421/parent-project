...
// Source File Name:   AddJspFields.java

package themes.ssh3.visitors;
    ...
    implements Visitor
{
    ...
    public Object visitTemplateElement(TemplateElement element)
    {
        ...
        for(int i = 0; i < voFields.length; i += 2)
        {
            ...
            buffer.append((new StringBuilder("<td><input type=\"text\" name=\"m.")).append(voFields[i]).append("\" ></td>\n\t").toString());
            if(i < voFields.length - 1)
            {
                buffer.append((new StringBuilder("<td>")).append(voFieldsNames[i + 1]).append("</td>\n\t").toString());
                buffer.append((new StringBuilder("<td><input type=\"text\" name=\"m.")).append(voFields[i + 1]).append("\"></td>\n").toString());
            }
            buffer.append("</tr>\n");
        }
